...
int32_t protocol_init(void)
{
    ...
    set_computer_value(SEND_TARGET_CMD, CURVES_CH1, &pid.target_val, 1);
    set_computer_value(SEND_PERIOD_CMD, CURVES_CH1, &pid.ducy_ms, 1);
    ...
    set_computer_value(SEND_P_I_D_CMD, CURVES_CH1, &val, 3);
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  //HAL_UART_Transmit(&UartHandle, (uint8_t *)&sum, sizeof(sum), 0xFFFFF);                  // 发送校验和
    ...
    rt_device_write(serial, 0, (uint8_t *)&set_packet, sizeof(set_packet));
    rt_device_write(serial, 0, (uint8_t *)data, num);
    rt_device_write(serial, 0, (uint8_t *)&sum, sizeof(sum));
}
